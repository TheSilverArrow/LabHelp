...
const TalongTab: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const aliases: Record<string, string[]> = {
      ...
      'mtb-rif (sputum)': ['sputum mtbrif', 'mtb-rif', 'mtb rif'],
      'genexpert (sputum)': ['genexpert sputum', 'gene xpert sputum', 'sputum genexpert'],
      'gs/cs (sputum)': ['sputum gscs', 'gscs sputum', 'sputum gs/cs', 'sputum gs cs', 'gscs', 'gs/cs', 'gs cs', 'culture and sensitivity'],
    ...
    matchedTests.forEach(test => {
      ...
      const isSpecial = nameUpper.includes('BIOFIRE') || nameUpper.includes('GENEXPERT') || 
                        nameUpper.includes('MTB/RIF') || nameUpper.includes('MTB-RIF') || 
                        nameUpper.includes('4PLEX');
      ...
      {mode === 'generator' && (
        <>
          ...
          <div id="preview-instructions" className="bg-white dark:bg-slate-800 border border-[#ced4da] dark:border-slate-700 p-[15px] rounded-lg my-5 shadow-[0_6px_15px_rgba(0,0,0,0.05)]">
            ...
            <p className="text-base mt-1.25 text-[#374151] dark:text-slate-300">
              <strong>• Printing:</strong> Leave scale at 100%. Form dimensions are configured so that 4 requests naturally fit on 1 A4 sheet with minimal margins.
            </p>
            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-700 flex items-center gap-3">
              <p className="text-xs text-gray-500 dark:text-slate-400 italic">Ensure paper size is A4 with minimal margins in the print preview (keep scale at 100%).</p>
            </div>
          </div>
